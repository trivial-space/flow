...
function create() {
  ...
  function touchEntity(eE, level = 0, eP) {
    if (!touchedEntities[eE.id]) {
      touchedEntities[eE.id] = {level}
    }

    let touch = touchedEntities[eE.id]
  ...
  function autostart(eP) {
    if (eP.async) {
      setTimeout(function() {
        execute(eP)
        delete executedProcesses[eP.id]
      }, 10)
    } else {
      execute(eP)
      touchEntity(eP.out)
  ...
  function start(processId) {
    ...
    if (eP.out && !eP.async) {
      ...
      delete executedProcesses[processId]
    }
  }
